    ...
    function insert_hechos($tipo, &$contenido)
    {
        ...
        switch ($tipo)
        {
            case 'implementaciones_alumnos':
                ...
                break;
            case 'implementaciones_docentes':
                if (isset($contenido['hechos'][$tipo]))
                {
                    $datos = $contenido['hechos'][$tipo];
                    foreach ($datos as $value)
                    {
                        $data = array(
                            'id_unidad_insituto' => $value[0],
                            'id_implementacion' => $value[1],
                            'id_rol_academico' => $value[2],
                            'id_sexo' => $value[3],
                            'cantidad_profesores' => $value[4],
                            'id_carga_informacion' => $id_carga
                        );
                        $this->db->insert('hechos.hechos_implementaciones_docentes', $data);
                    }
                }
                break;
            case 'unidades_instituto':
                if (isset($contenido['hechos'][$tipo]))
                {
                    $datos = $contenido['hechos'][$tipo];
                    foreach ($datos as $value)
                    {
                        $data = array(
                            'id_unidad_insituto' => $value[0],
                            'id_categoria' => $value[1],
                            'tipo_nomina' => $value[2],
                            'id_sexo' => $value[3],
                            'cantidad' => $value[4],
                            'id_carga_informacion' => $id_carga
                        );
                        $this->db->insert('hechos.hechos_unidades_instituto', $data);
                    }
                }
                break;
        }
